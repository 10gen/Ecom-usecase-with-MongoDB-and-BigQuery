...
const debug = require('@google-cloud/debug-agent').start({allowExperssions: true});
 ...
 * @param {Object} res Cloud Function response context.
 */
exports.getUserCluster = async (req, res) => {
  ...
  // const filename = "YOUR_PREDICTION_FILE_NAME";
   const endpointId = "1981223196229107712";
   const project = 'ecomm-analysis';
   const location = 'us-central1';
   //let userId;
   let session_count;
   let total_time_spend_by_user_in_msec;
   let average_order_value;
   let no_of_orders;

   ({session_count} = req.body);
   ({total_time_spend_by_user_in_msec} = req.body);
   ({average_order_value} = req.body);
   ({no_of_orders} = req.body);
 ...
 const {PredictionServiceClient} = require('@google-cloud/aiplatform');
  ...
  async function predictCustomTrainedModel() {
   ...
   const instance = {
      structValue: {
        fields: {
          average_order_value: {numberValue: average_order_value},
           no_of_orders: {numberValue: no_of_orders},
           session_count: {numberValue: session_count},
           total_time_spend_by_user_in_msec: {numberValue: total_time_spend_by_user_in_msec},
        },
      },
    };
